...
[![Version](https://img.shields.io/badge/version-0.6.1-blue)](https://github.com/HengQuWorld/CodeWhale-VSCode)
...
```bash
code --install-extension ./brotherwhale-vscode-0.6.1.vsix --force
```

> **Trae CN 用户：** 如果 `code` 不在 `PATH` 中，使用自带的 CLI：
> ```bash
> "/Applications/Trae CN.app/Contents/Resources/app/bin/code" --install-extension ./brotherwhale-vscode-0.6.1.vsix --force
> ```
...
**安装 VSIX**
```bash
code --install-extension /path/to/brotherwhale-vscode-0.6.1.vsix --force
```
