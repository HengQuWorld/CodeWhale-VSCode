...
- **The Changes panel now says what it cannot list** — Its rows come from the engine's file-change records, and only the file tools produce those: a file written by a shell command is recorded as a command execution, so a turn whose edits all came from a script read as a turn that changed nothing — the panel showed nothing for it, with no hint that anything had happened. A turn that ran shell commands is now listed with a line saying how many, and the panel itself says once, above the list, that it reports only the changes the file tools made. That holds whether the conversation is running or was opened from the Sessions rail. (Editing files with the file tools is still what makes a change reviewable row by row, with diff, locate and revert.)
...
- **A new conversation keeps the provider you picked, not whichever model another provider last set** — The model a new thread started with came from the single global `brotherwhale.defaultModel`, while the thread's provider was left to whatever route the runtime happened to be on. Those are two answers to one question, and nothing compared them. Choosing a model under the DeepSeek route therefore set the model for every route: on the Zhipu (`bigmodel-cn`) route the next conversation was pinned to `deepseek-flash`, and Zhipu answered `400 模型不存在` on the first message. The same pairing failed the legacy DeepSeek Anthropic route before a token was spent — its route check reported a missing key the user *had* configured, in another table. `POST /v1/threads` now carries the pair the picker is on (`model_provider` plus the exact `model_provider_id`) together with the model, so a thread can no longer be created on one route with another route's model, and a thread being rebuilt after its store was cleared comes back on the route it was running on rather than the picker's. Tasks and goal threads are created the same way. A runtime older than `model_provider_id` on the catalog is unaffected: nothing is claimed about a route it does not publish.

- **The model picker remembers one model per provider route** — `brotherwhale.defaultModel` is one value for every provider, so `/model deepseek-flash` on one route moved what the *next* provider's new conversations started from, and switching provider did not move it at all: the picker chip showed the model the runtime had just resolved (`glm-5.3`) while the conversation about to be created used the other value — the disagreement was silent until the provider refused. Each route now remembers its own model under `brotherwhale.modelByProvider` (keyed by the route's exact id, so the two DeepSeek routes never share a slot). `/model` and the provider picker write there; a route with nothing remembered falls back to the model its own runtime catalog publishes, and only then to `defaultModel`. The chip, the model list and the conversation about to be created all read that one value.

- **The two DeepSeek routes in the provider picker are told apart, and routes that cannot run are not offered** — The Runtime publishes the DeepSeek API twice: the vendor route (`deepseek`) and the legacy Anthropic dialect route (`deepseek-anthropic`). Both were labelled "DeepSeek", both answered the same model catalog, and only one of them held the user's key — so "DeepSeek" could be selected twice with different results, and the failure only appeared on the first message of the next conversation. Each row now appends its route id when another row shares its display name (`DeepSeek (deepseek)` / `DeepSeek (deepseek-anthropic)`), and carries its credential state: a route with no key is left out of the picker entirely, the route that is already active stays listed and says `no key configured`, and one that only needs a login is offered with `needs login`. The same rules drive the config panel's Provider select. A runtime that does not publish credential states shows every row as before.

- **A viewed session's route and model reach the toolbar** — Opening a saved session to look at it (before anything resumes it) showed the session's model on the chip and the *picker's* provider beside it: the provider was never published for that state, so the toolbar named one route while the conversation on screen would run on its own the moment a message resumed it. The session's route and model now travel with it, leaving with it too (resume, delete, 新建会话).

- **The picker says what the switch does and does not reach** — A conversation keeps the provider it was created on: an engine decision, for prefix-cache economics (`provider_identity_for_thread` resolves the thread's own route, neither `PATCH /v1/threads` nor `POST .../turns` carries a provider, and `resume-thread` says a caller that needs a different route is creating a conversation, not resuming one). The toolbar said otherwise. After switching to DeepSeek the chip read *DeepSeek* and the model menu listed DeepSeek's models, while the next message still went to the Zhipu route — and picking `deepseek-flash` from that menu wrote it onto a thread pinned to `bigmodel-cn`, so the provider answered `400 模型不存在`; read from the outside, "DeepSeek also fails". The chip, the model menu and the model request now all describe the route the open conversation runs on (the picker's route only when there is no conversation), the chip says `· this conversation` while the two differ, and a switch with such a conversation open offers the one action that does move it — a new conversation — instead of letting the next message land somewhere the user did not choose. Nothing in the client tries to re-route an open conversation; that stays the engine's decision.

- **`/model` refuses a model that belongs to another provider route** — The model menu is scoped to the route on screen now, but `/model <id>` can still name any id at all, and an id another route is known to use is the pair a provider answers `400 模型不存在` for. That id is refused with the route it belongs to; an id the route merely does not list is left to the provider, because the engine accepts a name-shaped id for a route whose catalog is not exhaustive and a client stricter than its engine would reject models that work. The check reads only the catalog the client already holds, so `/model` never waits on the network to answer.

## 0.7.3

Everything below is client-side and verified against engine **v0.10.0**, the
current engine release. Two behaviours are only half the story on their own: the
client half ships here, the engine half is newer than v0.10.0 and is in no
published engine yet. Each is marked inline, and both are listed with their
status under **Upstream TUI PRs** at the end of this section.
...
- **The composer's steering has a button of its own, instead of living behind the Stop button's face** — While a turn streams, the box says "Steer the running turn..." and Enter guides it — but the only control in the composer was the red **Stop** button, so the one thing a mouse user could click was the one action that ends the turn they were trying to guide: the placeholder advertised one action and the button performed its opposite. Typing is a second action now, so it gets a second control: a **guide** button beside Stop, revealed only while a turn that can take guidance is running, dimmed while the box is empty (its tooltip says what it is waiting for), and wired to exactly the path Enter takes, so the button and the key cannot drift apart. Stop keeps stopping and no longer clears the box — it is not the sender any more — and a steer the engine refuses hands the text back to the box (the TUI restores a steer it never took, "so nothing is lost"; a refused send already recovers this way). On an engine without steering, the composer is unchanged: Stop alone. The box also stops promising what it cannot do there — typing goes nowhere on such an engine, and it now says so instead of falling back to "Type a message...".

- **A thread loaded while its turn is still running can be steered again** — Switching back to a thread with a turn in flight, opening one from the rail, or resuming a session into a thread that is already busy all adopt that turn, and the composer did not know it: the history rebuild ends the streaming state (it replaces the whole conversation), and the streaming placeholder the host posts *before* that draw was the only thing that armed it — only `addMessage` for a streaming message does. So plain text typed at a running turn started a new one instead of steering it, the engine refused it (`Thread already has an active turn`), the recovery adopted the turn again without arming anything, and the loop repeated: mid-turn steering was unreachable for exactly the turns a user had just come back to, and once a delta arrived the button read **Stop** while the click sent the prompt. The rebuild now reads the transcript it just drew — a trailing streaming assistant bubble *is* a turn the engine is still running — so the streaming state and the Stop button come back with it. The send/stop button follows that same state rather than the status line's last message, so `ready` and `settingsUpdated`, which arrive right after a load and only describe the model, can no longer hand a live turn's composer back to Send, and a turn's own error clears the state before it repaints the status line. An adopted turn arms the state itself, deadline included, so the first prompt after a refused send steers instead of being refused again — and a turn whose engine went silent still releases the composer rather than holding it as Stop forever.
...
- **A conversation keeps the session it saves to, instead of collecting one per visit** — The extension remembered which saved session a conversation writes to only in memory, and only on the two paths that set it explicitly: resuming a saved session, and `/save`. Opening a thread from the rail, coming back after a window reload, or parking a busy thread left the next auto-save with no session id — which is a request to create one — so a single conversation accumulated a session document per visit while the one it had been writing to was left behind with nothing referencing it. A measured store held 122 sessions for 64 bound threads, 58 of them unreferenced, and one conversation was spread over eight documents whose transcripts ran 390 → 438 → … → 753 messages; the Sessions list showed every one of them as an equal row, with nothing marking which was the live conversation. The binding is the runtime's and `GET /v1/threads/{id}` has always returned it — the client's type dropped it. Every path that opens a thread now reads it off the record, and every path that creates one drops it, so a new or rebuilt thread cannot save a transcript that never came from the document it still names. Two cases where trusting that binding would overwrite a document belonging to another thread are handled the same way, because saving *replaces* the stored transcript: an undo or retry fork inherits the source's session id on a runtime that predates the fork-owns-its-document fix, so it now drops that inheritance rather than writing its shorter history over the thread it came from — which would have left that thread unable to load at all — and a parked fork no longer re-derives a binding it deliberately dropped. Resuming a session that is already open, meanwhile, hands back the thread that holds it (`200`, not `201`): that thread's title is the user's, so the resume path only names a thread it actually created. (The session-id adoption takes effect against the runtime as it ships today; for a fork to *keep* a document rather than mint one on its first save, the runtime fix that gives a fork its own — TUI #6406 — has to be installed. That PR is open and unmerged, and is in no published engine.)

### Upstream TUI PRs

Status as of 2026-09-23, for the two behaviours above whose engine half is newer
than engine v0.10.0.

- [Codewhale#6404](https://github.com/Hmbown/Codewhale/pull/6404) — **merged 2026-09-23, not yet in any engine release.** `fix(api): expose user-defined [providers.<name>] routes to native clients`. The engine half of "A provider you configured in the TUI appears in the picker and can be selected". On engine v0.10.0 a `[providers.<name>]` route configured in the TUI is still absent from `GET /v1/providers`, so the picker cannot list or select it; the client side is in place and starts working the moment an engine carrying this is installed.

- [Codewhale#6406](https://github.com/Hmbown/Codewhale/pull/6406) — **open, unmerged.** `fix(tui): stop resume and fork from duplicating threads and sessions`. The engine half of the fork case in "A conversation keeps the session it saves to": without it, a fork mints a session document of its own on its first save rather than keeping the one it was bound to. The rest of that fix — the binding read off `GET /v1/threads/{id}`, and dropping an inherited binding rather than overwriting another thread's document — works against engine v0.10.0 as it ships.
