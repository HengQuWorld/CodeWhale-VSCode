...
```bash
code --install-extension ./brotherwhale-vscode-0.7.0.vsix --force
```

> **Trae CN users:** if `code` is not on your `PATH`, use the bundled CLI:
> ```bash
> "/Applications/Trae CN.app/Contents/Resources/app/bin/code" --install-extension ./brotherwhale-vscode-0.7.0.vsix --force
> ```
...
**Installing a VSIX**
```bash
code --install-extension /path/to/brotherwhale-vscode-0.7.0.vsix --force
```
